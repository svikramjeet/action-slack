...
if (argv._.length === 0) {
  console.error(
    `Cannot send empty message. Please fill the 'args' field of the GitHub Action.`
  );
  process.exit(1);
}
...
if (process.env.SLACK_OVERRIDE_MESSAGE) {
  if (
    process.env.SLACK_OVERRIDE_MESSAGE === true ||
    process.env.SLACK_OVERRIDE_MESSAGE === "true"
  ) {
    fullMessage = message;
  } else {
    fullMessage = _.template(process.env.SLACK_OVERRIDE_MESSAGE)({
      ...process.env,
      EVENT_PAYLOAD
    });
  }
...
(async () => {
  ...
  await axios.post(
    url,
    querystring.stringify({ payload: JSON.stringify(payload).replace(/\\\\n/gm, '\\n') }),
    {
      headers: {
